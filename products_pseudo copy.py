...
def run(args, device, data, tic):
	...
	for epoch in range(args.num_epochs):
		...
		for full_batch_step, (input_nodes, output_seeds, full_batch_blocks) in enumerate(full_batch_dataloader):
			...
			ssss_time = time.time()
			block_dataloader = generate_dataloader(train_g, full_batch_blocks, args)
			...
			for step, (input_node, seeds, blocks) in enumerate(block_dataloader):
				...
				tic_step = time.time()
			# print('the current epoch throughput ' )
			# print(iter_tput)
			...
			if epoch % args.log_every==0:
				...
				print(
					'Epoch {:05d} | Step {:05d} | Loss {:.4f} | Train Acc {:.4f} | Speed (samples/sec) {:.4f} | GPU {:.1f} MB'.format(
						epoch, 0, pseudo_mini_loss.item(), acc.item(), np.mean(iter_tput), gpu_mem_alloc))
		...
		print('Avg cpu epoch time: {} ms'.format(np.mean(avg )* 1000))
...
#-------------------------------------------------------------------------------------------------------
	...
	print('test data loader model cost time: ' + str(time.time() - test_time))
